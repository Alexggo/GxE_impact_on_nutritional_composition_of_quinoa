...
Los grupos Tukey estan ya en la tabla.

```{r include=FALSE}
#Estadistica de rawTallos
rawtT<-read.csv("rawTallos.csv")
...
lm.elem<-list()
for (i in 1:7){
  lm.elem[[i]]<-lm(CONCENTRATION~VARLOC,data=subrawelem[[i]])
}
names(lm.elem)<-elemvet
...
marginal<-list()
for (i in 1:7){
  marginal[[i]] <- emmeans(lm.elem[[i]], "VARLOC")
}
...
```
Hacer otro test de comparaciones multiples, como duncan.test()
